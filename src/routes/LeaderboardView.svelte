...
<div class="screen p-4 max-w-md mx-auto min-h-screen bg-white">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">🏆 សមាជិកឆ្នើម</h2>
        ...
        {#each leaderboard as user, i}
            <div class="flex items-center bg-white p-3 rounded-xl shadow-sm border border-gray-200">
                <div class="font-bold text-xl w-8 text-center mr-2
                    {i === 0 ? 'text-primary' : i === 1 ? 'text-gray-600' : i === 2 ? 'text-gray-500' : 'text-gray-400 opacity-50'}">
                    #{i + 1}
                </div>
                ...
                <div class="font-bold text-primary">{user.xp} XP</div>
            </div>
        {/each}
